  ...
  const onSubmit = async (data) => {
    ...
    try {
      const user = await authRegister({
        ...
        role: 'user'   // always user — agents apply via /agent-apply
      });
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-12 overflow-y-auto">
        <div className="w-full max-w-md">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-border p-8">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input {...register('confirmPassword', { required: 'Please confirm password', validate: v => v === password || 'Passwords do not match' })}
                    type="password" placeholder="Repeat your password" className="input-field pl-9" />
                </div>
            ...
            <p className="text-center text-text-sub text-xs mt-3">
              Are you an agent?{' '}
              <Link to="/agent-apply" className="text-primary font-medium hover:underline">Apply here</Link>
            </p>
          </div>
        </div>
      </div>
