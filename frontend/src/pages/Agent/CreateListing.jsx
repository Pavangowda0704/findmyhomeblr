...
export default function CreateListing() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      // Area
      const area = { total: data.totalArea, carpet: data.carpetArea, unit: data.areaUnit || 'sqft' };
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-dark">Add New Listing</h1>
        <p className="text-text-sub mt-1">Fill in the details to list your property</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="space-y-5">
            ...
            <div className="grid sm:grid-cols-2 gap-5">
              <div>
                ...
                <select {...register('listingType', { required: true })} className={inputClass}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            {AMENITIES.map(a => (
              <label key={a.value} className="cursor-pointer">
                ...
                <div className={`flex items-center gap-2 px-3 py-2.5 rounded-lg border text-sm transition-all ${
                  selectedAmenities.includes(a.value)
                    ? 'bg-primary/10 border-primary text-primary font-medium'
                    : 'bg-background border-border text-text-sub hover:border-primary'
                }`}>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-3 mt-5">
              {previews.map((src, i) => (
                <div key={i} className="relative group aspect-square rounded-xl overflow-hidden border border-border">
                  ...
                  {i === 0 && (
                    <div className="absolute top-2 left-2 bg-primary text-dark text-xs font-bold px-2 py-0.5 rounded-full">Cover</div>
                  )}
                  <button
                    type="button"
        ...
        <div className="flex gap-4">
          ...
          <button type="button" onClick={() => navigate('/agent/listings')} className="btn-outline py-3 px-8">
            Cancel
          </button>
        </div>
      </form>
